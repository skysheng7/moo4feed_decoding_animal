###################################################################################################
############### 12a-12. Predictability — number_of_non_nutritive_visits (interactive session) ######
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_number_of_non_nutritive_visits.rds")

cat("Fitting number_of_non_nutritive_visits ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: number_of_non_nutritive_visits saved to", rds_path, "\n")
} else {
  cat("FAILED: number_of_non_nutritive_visits --", err, "\n")
...
if (length(warns) > 0) {
  ...
  if (length(warns) > 0) {
    cat("\nWarnings for number_of_non_nutritive_visits:\n")
