###################################################################################################
############### 12a-09. Predictability — median_visit_per_meal (interactive session) ###############
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_median_visit_per_meal.rds")

cat("Fitting median_visit_per_meal ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: median_visit_per_meal saved to", rds_path, "\n")
} else {
  cat("FAILED: median_visit_per_meal --", err, "\n")
...
if (length(warns) > 0) {
  ...
  if (length(warns) > 0) {
    cat("\nWarnings for median_visit_per_meal:\n")
