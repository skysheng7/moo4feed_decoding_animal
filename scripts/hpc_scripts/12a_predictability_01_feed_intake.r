###################################################################################################
############### 12a-01. Predictability — feed_intake (interactive session) #########################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_feed_intake.rds")

cat("Fitting feed_intake ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: feed_intake saved to", rds_path, "\n")
} else {
  cat("FAILED: feed_intake --", err, "\n")
