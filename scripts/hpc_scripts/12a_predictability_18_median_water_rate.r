###################################################################################################
############### 12a-18. Predictability — median_water_rate (interactive session) ###################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_median_water_rate.rds")

cat("Fitting median_water_rate ...\n")

f <- bf(
  median_water_rate ~ DIM + parity + THI_mean + poly(month, 2) + (1 | cow),
  sigma ~ (1 | cow)
)
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: median_water_rate saved to", rds_path, "\n")
} else {
  cat("FAILED: median_water_rate --", err, "\n")
...
if (length(warns) > 0) {
  ...
  if (length(warns) > 0) {
    cat("\nWarnings for median_water_rate:\n")
