###################################################################################################
############### 12a-02. Predictability — feed_duration (interactive session) #######################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_feed_duration.rds")

cat("Fitting feed_duration ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: feed_duration saved to", rds_path, "\n")
} else {
  cat("FAILED: feed_duration --", err, "\n")
