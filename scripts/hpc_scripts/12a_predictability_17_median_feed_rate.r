###################################################################################################
############### 12a-17. Predictability — median_feed_rate (interactive session) ####################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_median_feed_rate.rds")

cat("Fitting median_feed_rate ...\n")

f <- bf(
  median_feed_rate ~ DIM + parity + THI_mean + poly(month, 2) + (1 | cow),
  sigma ~ (1 | cow)
)
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: median_feed_rate saved to", rds_path, "\n")
} else {
  cat("FAILED: median_feed_rate --", err, "\n")
