###################################################################################################
############### 12a-06. Predictability — water_visits (interactive session) ########################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_water_visits.rds")

cat("Fitting water_visits ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: water_visits saved to", rds_path, "\n")
} else {
  cat("FAILED: water_visits --", err, "\n")
