###################################################################################################
############### 12a-04. Predictability — water_intake (interactive session) ########################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_water_intake.rds")

cat("Fitting water_intake ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: water_intake saved to", rds_path, "\n")
} else {
  cat("FAILED: water_intake --", err, "\n")
