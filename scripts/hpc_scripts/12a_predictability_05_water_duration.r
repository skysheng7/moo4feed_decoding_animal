###################################################################################################
############### 12a-05. Predictability — water_duration (interactive session) ######################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_water_duration.rds")

cat("Fitting water_duration ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: water_duration saved to", rds_path, "\n")
} else {
  cat("FAILED: water_duration --", err, "\n")
