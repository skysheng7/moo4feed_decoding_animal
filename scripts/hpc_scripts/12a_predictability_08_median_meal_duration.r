###################################################################################################
############### 12a-08. Predictability — median_meal_duration (interactive session) ################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_median_meal_duration.rds")

cat("Fitting median_meal_duration ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: median_meal_duration saved to", rds_path, "\n")
} else {
  cat("FAILED: median_meal_duration --", err, "\n")
...
if (length(warns) > 0) {
  ...
  if (length(warns) > 0) {
    cat("\nWarnings for median_meal_duration:\n")
