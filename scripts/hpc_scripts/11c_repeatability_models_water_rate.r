###################################################################################################
######################### 11c. Repeatability — Water Rate (HPC) ##################################
###################################################################################################
# Prerequisites: run 11a_repeatability_setup.r first
#
# Single-model script for median_water_rate, intended for HPC submission.
...
rds_path <- file.path(output_dir, "m1_brm_median_water_rate.rds")

###################################################################################################
################################## Fit model ######################################################
###################################################################################################
message("Fitting median_water_rate ...")

warns <- list()
model <- withCallingHandlers(
  brm(
    formula = median_water_rate ~ DIM + parity + THI_mean + poly(month, 2) + (1 | cow),
    data    = master_data,
    family  = lognormal(),
    warmup  = 1000,
    iter    = 15000,
...
message("Done: median_water_rate — saved to ", rds_path)
