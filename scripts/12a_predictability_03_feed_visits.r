###################################################################################################
############### 12a-03. Predictability — feed_visits (interactive session) #########################
###################################################################################################
...
rds_path <- file.path(output_dir, "m2_brm_feed_visits.rds")

cat("Fitting feed_visits ...\n")
...
if (!is.null(model)) {
  ...
  cat("SUCCESS: feed_visits saved to", rds_path, "\n")
} else {
  cat("FAILED: feed_visits --", err, "\n")
