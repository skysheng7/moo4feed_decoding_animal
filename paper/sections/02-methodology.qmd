...
**Data Processing, Configuration and Quality Control.** Raw visit data were converted into a standardized format, with each record capturing the cow's identity, the bin number, visit start and end times, and the start and end weights of feed or water in the bin. Only days for which both feeder and drinker data files were available were retained. Daylight saving time transitions were detected and corrected automatically based on the timezone. During fall-back transitions (when clocks move back one hour, causing chronological order of visits within the repeated hour to be ambiguous), we removed all visits from the duplicated hour, since it is not possible to determine the true chronological order, and adjusts subsequent timestamps accordingly. During spring-forward transitions (when clocks move forward by one hour), visit timestamps are shifted forward by one hour for the remainder of the day. Records were then passed through a sequence of automated quality control checks, summarized in Table 1.

```{=latex}
\newpage
```

**Table 1.** Automated quality control checks applied to raw visit data from electronic feed and water bins. Dashes (—) indicate that threshold id is not applicable or is data-driven, no hard-coded numerical threshold applies. Action: Flag = abnormal record is retained and sent to researchers on the farm to review for bin health check; Correct = the record is automatically corrected; Remove = the record is deleted from the dataset. For negative weight corrections, intake is recalculated from the corrected weights (start weight − end weight); if the recalculated intake remains negative, the record is removed.
...
Flagged records were compiled into a daily report and sent to the researchers and students working on the farm at the time of data collection, alerting them to inspect the electronic bins and record any abnormal events (manually noted events are listed in Section 2.2). Following these initial checks, multivariate outlier detection was performed using a k-nearest neighbor (KNN) algorithm [@malini2017analysis] applied to scaled visit duration, intake, and intake rate. Unlike rectangular cutoffs based on standard deviations of each variable independently, the KNN approach operates in multidimensional space and flags visits that are unusual in combination even when plausible on any single dimension. In this study, k = 50, and the flagging thresholds were set such that 0.04% of feed visits and 0.1% of water visits were identified as outliers.
