...
To process the raw feeding and drinking data and extract meaningful behavioral variables, we developed an open-source R package, 'moo4feed' (https://www.skysheng.io/moo4feed/), which provides a standardized, customizable workflow for handling electronic feed and water bin data. The package supports five areas of analysis used in this study: (1) data processing, configuration, and quality control; (2) agonistic replacement detection; (3) non-nutritive visit detection; (4) feed availability estimation; and (5) meal clustering. All thresholds and processing parameters described below are user-configurable to cater to different farm setups; the values reported here are those used in the present study. Full documentation, including additional functionalities not used here (e.g., pairwise synchronicity analysis), please refer to the package website.


```{r}
#| label: tbl-qc-checks
#| tbl-cap: "Automated quality control checks applied to raw visit data from electronic feed and water bins. Dashes (—) indicate that threshold id data-driven, no hard-coded numerical threshold applies. Action: Flag = abnormal record is retained and marked for researchers to review in the daily quality control report; Correct = the record is automatically corrected; Remove = the record is deleted from the dataset. For negative weight corrections, intake is recalculated from the corrected weights (start weight − end weight); if the recalculated intake remains negative, the record is removed. All flagged records are retained in the final dataset; the flagging procedure functions as a warning system rather than a data filter."
#| echo: false

qc_tbl <- tibble::tribble(
  ~Check,                            ~Description,                                                                                                                         ~`Feed threshold`, ~`Water threshold`, ~Action,
  "Expected cow count",              "Fewer cows detected than expected, indicating potential lost ear tags",                                                               "\u2014",          "\u2014",           "Flag",
  ...
  "Large single-visit intake",       "Unrealistically large intake per visit",                                                                                              ">8 kg",           ">30 L",            "Flag",
  "No afternoon feeding or drinking","Cow did not feed or drink after noon, potentially due to a lost ear tag or illness",                                                  "\u2014",          "\u2014",           "Flag",
  "Low or no bin traffic",           "Bins with unusually low traffic or no visits, indicating potential equipment malfunction",                                             "\u2014",          "\u2014",           "Flag",
  "Long visit duration",             "Implausibly long visit duration",                                                                                                     ">2,000 s",        ">1,800 s",         "Flag",
  "High intake rate",                "Excessively high intake rate per visit",                                                                                              ">0.008 kg/s",     ">0.35 L/s",        "Flag",
  "Low daily visit count",           "Fewer than 10 visits recorded, suggesting a potentially incomplete day",                                                              "<10 visits",      "<10 visits",       "Flag",
...
flextable(qc_tbl) |>
  ...
  theme_booktabs() |>
  align(j = "Check",           align = "left",   part = "body") |>
  align(j = "Description",     align = "left",   part = "body") |>
  align(j = c("Feed threshold", "Water threshold", "Action"),
...
Flagged records were compiled into a daily report and sent to the researchers and students working on the farm at the time of data collection, alerting them to inspect the electronic bins and record any abnormal events (manually noted events are listed in Section 3.2). Following these initial checks, multivariate outlier detection was performed using a k-nearest neighbor (KNN) algorithm [@malini2017analysis] applied to scaled visit duration, intake, and intake rate. Unlike rectangular cutoffs based on standard deviations of each variable independently, the KNN approach operates in multidimensional space and flags visits that are unusual in combination even when plausible on any single dimension. In this study, k = 50, and the flagging thresholds were set such that 0.04% of feed visits and 0.1% of water visits were identified as outliers.

After cleaning, feed and water datasets were combined, and six conventional daily behavioral variables were calculated per cow: total daily feed intake (kg), total daily feeding duration (s), total feeding visits, total daily water intake (L), total daily drinking duration (s), and total drinking visits.
...
In total, 16 behavioral variables were included in the statistical analyses described in Section 3.4: six basic daily feeding and drinking variables (i–vi), two daily agonistic variables (vii–viii), one non-nutritive visit variable (ix), one feed availability variable (x), and six meal-level variables (xi–xvi). These variables were selected because they capture biologically meaningful aspects of individual feeding, drinking, exploration and social strategies. We do not claim that this set is exhaustive; other variables could be derived from the same data streams. The 'moo4feed' package and the analytical framework described in Section 3.4 are designed to accommodate additional variables, and we encourage future researchers to extend this set and evaluate which further metrics may be informative for individual profiling.
...
**Behavioral variable clustering.** To organize the 16 behavioral variables by how informative they are for individual profiling, we applied k-means clustering (k = 3, 50 random starts) to the posterior mean CVi and R values for each variable, after scaling both dimensions. All variables have low predictaibility values, thus measures of predictability were not used in behavioral variable clustering. The number of clusters (k = 3) was determined by researcher judgment based on visual inspection of the CVi and R distributions. The three clusters were labeled by descending R and CVi: Cluster 1 (high R, high CVi: traits in which cows differ substantially in their individual average behavioral expression relative to the herd mean, and those individual differences are consistent over time); Cluster 2 (high R, low CVi: traits in which individual differences are consistent over time, but the variation among cows is small relative to the herd mean); and Cluster 3 (low R, low CVi: traits in which cows show little variation relative to the herd mean, and what little variation exists is not consistent over time). We used only the variables in Cluster 1 for the downstream cow clustering, as they were deemed as the most informative for individual cow profiling.

**Individual-level behavioral profiles.** For each cow and each behavioral variable in Cluster 1, we extracted the individual intercept from the posterior distribution: the population intercept plus each cow's random effect (back-transformed to the original scale for lognormal models). This represents a cow's average behavioral type (i.e., a cow's typical expression of their behavior). For example, for daily feed intake modeled with a Gaussian likelihood, if the population intercept is 50 kg and cow A's random effect is 20 kg, cow A's individual intercept would be 70 kg, representing her typical daily feed intake after accounting for DIM, parity, THI, and seasonal effects.
